...
**CodexPad-C10​** 是 CodexPad 系列中专为创客与嵌入式开发者设计的低功耗蓝牙手柄。与市面上依赖操作系统蓝牙协议栈的通用手柄不同，本产品主打**低门槛接入裸机及轻量级运行时环境**，无需厚重系统层支持，即可与各类低功耗蓝牙硬件平台（如 **ESP32** 系列、**ESP32-S** 系列、**ESP32-C** 系列、**STM32** 系列、**nRF** 系列、 **micro:bit**、**树莓派**系列等）进行对等通信，为机器人、物联网设备、自定义控制面板等提供了即插即用的远程物理输入解决方案。

我们为适配的硬件平台提供了简洁的通信协议、轻量级驱动库与丰富示例，让开发者能快速将手柄集成到固件中，专注于核心功能的实现。

> **⚠️ 这不是一款"即插即用"的通用游戏手柄**
>
> CodexPad-C10 **不是 BLE-HID 设备**，它不会像 Xbox、PlayStation 等标准手柄那样被操作系统自动识别为标准游戏控制器。因此：
>
> - **❌ 不能**直接连上 Windows / macOS / Linux 或手机、游戏主机就去打游戏；
> - **✅ 必须**通过**代码手动建立蓝牙连接**，并自行解析手柄上报的数据（按键、摇杆等）来获取输入。
>
> 换句话说，**任何输入都需要由你的程序主动读取和处理**，系统不会帮你完成"按键→键盘/手柄事件"的转换。本产品的定位是**开发用途的输入设备**，而非面向普通玩家的即插即用游戏外设。
...
> **说明**：下表所列均为官方已提供适配库与示例代码的硬件平台，开发者可直接使用对应资源进行快速开发。<br>
> 理论上，任何具备标准 BLE 能力的硬件平台均可与本手柄通信。若您使用的平台未在列表中，可参考已有平台的库实现与通信协议进行自行移植；官方也将根据市场需求与技术规划，持续评估并扩展对新平台的支持。
...
| 硬件组合 | 开发环境与语言 |
| :--- | :--- |
| <ul><li>Arduino UNO + NL16 蓝牙模块</li></ul> | <ul><li>Arduino IDE（C/C++）</li></ul> |
| <ul><li>BLE UNO（内置 NL16 模块）</li></ul> | <ul><li>Arduino IDE（C/C++）</li></ul> |
